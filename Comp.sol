...
contract Comp is Ownable {
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, _computeVote(srcRep, dstRep, srcRepOld, amount), "Comp::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, _computeVote(srcRep, dstRep, amount), "Comp::_moveVotes: vote amount overflows");
    ...
    function _computeVote(address srcRep, uint96 dstRep, uint96 amount) internal returns (uint96 computedVotes) {

        if (voteShimAddress == address(0)) {
            return newVotes;
        }
        computedVotes = IVoteShim(voteShimAddress).computeVote(srcRep, dstRep, amount);
